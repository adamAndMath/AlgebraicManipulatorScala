...
case class ProofTemplate(header: Header, result: List[Exp], count: Int, origin: Exp, manipulations: List[Manipulation]) extends IdentityTemplate {
  override def apply(finder: Finder): Proof = {
    ...
    (manipulations.indices zip manipulations).foreach{case (i, m) => try {proof(finder, m)} catch { case e: Exception => throw new IllegalStateException(s"Failed to apply manipulation ${i+1}: $m for ${proof.current.mkString("=")}", e)}}
    proof
  }
