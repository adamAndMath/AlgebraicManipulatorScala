...
case class Unwrap(positions: Tree) extends PathManipulation(positions) {
  ...
  override def replace(env: Environment, exp: Exp): Exp = exp match {
    case Variable(name) => env.find(List(name)).asInstanceOf[Wrapable].unwrap(exp)
    case Operation(Variable(name), _) => env.find(List(name)).asInstanceOf[Wrapable].unwrap(exp)
